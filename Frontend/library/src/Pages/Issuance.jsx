...
function Issuance() {
    ...
    const postIssuance = async (event) => {
        ...
        try {
            const newIssuance = await axios.post("http://localhost:3001/issuance", {
                book_id: bookid,
            ...
            console.log("Issuance created successfully:", newIssuance.data);
            Nextpage("/");
        } catch (err) {
            console.error("PostIssuance error:", err.response?.data || err.message);
        }
    };
    ...
    return (
        <div className="form-container">
            ...
            <button onClick={postIssuance}>Add Issuance</button>
        </div>
    );
}
